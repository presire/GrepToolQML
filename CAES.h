#ifndef CAES_H
#define CAES_H

#include <QtCore>

class CAES
{
private:
    enum
    {
        m_NB = 4,   /* 128bit 固定として規格されている(データの長さ) */
        m_NBb = 16
    };

    int m_NK;           // 鍵の長さ 4, 6, 8(128, 192, 256bit)
    int m_NR;           // ラウンド数 10, 12, 14
    int m_KeyLength;    // 鍵長
    unsigned char m_key[32] = {};

    const unsigned char m_Keys[32] = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,
                                      0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f,
                                      0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,
                                      0x18,0x19,0x1a,0x1b,0x1c,0x1d,0x1e,0x1f};

    /* FIPS 197  P.16 Figure 7 */
    int Sbox[256]={
      
    };

    /* FIPS 197  P.22 Figure 14 */
    int invSbox[256]={
      
    };

public:

private:
    void    SubBytes(int data[]);            /* FIPS 197  P.16 Figure  6 */
    void    ShiftRows(int data[]);           /* FIPS 197  P.17 Figure  8 */
    void    MixColumns(int *);               /* FIPS 197  P.18 Figure  9 */
    void    AddRoundKey(int *,int);          /* FIPS 197  P.19 Figure 10 */
    int     SubWord(int in);                  /* FIPS 197  P.20 Figure 11 */ /* FIPS 197  P.19  5.2 */
    int     RotWord(int in);                  /* FIPS 197  P.20 Figure 11 */ /* FIPS 197  P.19  5.2 */
    void    KeyExpansion(void *);            /* FIPS 197  P.20 Figure 11 */
    void    invShiftRows(int data[]);        /* FIPS 197  P.22 Figure 13 */
    void    invSubBytes(int data[]);         /* FIPS 197  P.22 5.3.2 */
    void    invMixColumns(int data[],int n); /* FIPS 197  P.23 5.3.3 */
    int     mul(int dt,int n);
    int     dataget(void* data,int n);

#ifdef QT_DEBUG
    void    datadump(const char c[], void *dt, int len);
#endif

public:
    CAES(const char *strAES);
    ~CAES();

    QByteArray Crypt(const QByteArray &ByaryPlainData);     /* FIPS 197  P.15 Figure  5 */
    QByteArray DeCrypt(const QByteArray &ByaryEncryptData); /* FIPS 197  P.21 Figure 12 */

#ifdef QT_DEBUG
    int Test(const QByteArray &ByaryPlainData);
    int Cipher(int data[]);                                 /* FIPS 197  P.15 Figure  5 */
    int invCipher(int *);                                   /* FIPS 197  P.21 Figure 12 */
#endif
};


#endif // CAES_H
